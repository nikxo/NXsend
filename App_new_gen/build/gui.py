...
from tkinter import Frame, Scrollbar, HORIZONTAL
...
# Création d'un cadre pour contenir les boutons à l'intérieur du Canvas
container_frame = Frame(canvas, bg="#FFFFFF")  # Création d'un cadre
canvas.create_window((0, 0), window=container_frame, anchor='nw')


# Fonction pour ajuster la région de défilement du Canvas lorsque les boutons sont ajoutés
def configure_scroll_region(event):
    canvas.configure(scrollregion=canvas.bbox("all"))


# Configuration de la fonction pour ajuster la région de défilement lors d'un changement de taille
container_frame.bind("<Configure>", configure_scroll_region)
...
button_y_position = 0  # Variable pour suivre la position Y des boutons


def create_rounded_button():
    global button_y_position  # Utilisation de la variable globale

    custom_button = customtkinter.CTkButton(
        canvas,  # Utilisation du cadre passé en argument
        text="",
    ...
    custom_button.place(x=0, y=button_y_position)
...
add_boutton = customtkinter.CTkButton(
    window,
    text="",
    command=add_button,
    width=168,
...
add_boutton.place(x=5, y=485)  # Position du bouton
